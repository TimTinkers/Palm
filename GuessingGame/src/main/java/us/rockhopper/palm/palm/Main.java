...
public class Main {
	...
	public static void main(final String[] args) {
		...
		System.out.println("Which token are you going to play with?");
		BigInteger tokenID =
				BigInteger.valueOf(Long.parseLong(scanner.nextLine()));
		int guessCount = Integer.parseInt(tokenMetadata(tokenID));
		if (guessCount == 0) {
			System.out.println(
					"You don't have any guesses with that token! Go try out the shooting gallery first.");
			System.exit(0);
		} else if (guessCount == 1) {
			System.out.println(
					"Alright, given your high score on the shooting gallery you have 1 guess!");
		} else {
			System.out.println(
					"Alright, given your high score on the shooting gallery you have "
							+ guessCount + " guesses!");
		}

		// Accept a guess.
		Random random = new Random();
		int correctValue = random.nextInt(101);
		...
		while (guessCount > 0) {
			...
			if (guess == correctValue) {
				...
				System.exit(0);
			} else if (Math.abs(guess - correctValue) < 10) {
				System.out.println("Nope, but you're not far wrong!");
			} else {
				System.out.println("Nope, that wasn't it!");
			}
			guessCount--;
		}
		System.out.println("Sorry, you're out of guesses! I was thinking "
				+ correctValue + ".");
		System.exit(0);
		scanner.close();
	}
